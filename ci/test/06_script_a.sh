...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  ...
  CI_EXEC ./autogen.sh
  CI_EXEC ./configure "$GROESTLCOIN_CONFIG_ALL" "$GROESTLCOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)
  CI_EXEC "make $MAKEJOBS && cd src/qt && ANDROID_HOME=${ANDROID_HOME} ANDROID_NDK_HOME=${ANDROID_NDK_HOME} make apk"
  exit 0
fi

GROESTLCOIN_CONFIG_ALL="${GROESTLCOIN_CONFIG_ALL} --enable-external-signer --bindir=$BASE_OUTDIR/bin --libdir=$BASE_OUTDIR/lib"
