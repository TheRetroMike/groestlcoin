#!/usr/bin/env bash
#
# Copyright (c) 2019-2023 The Groestlcoin Core developers
...
PRUNED=$( ${GROESTLCOIN_CLI_CALL} getblockchaininfo | awk '/pruneheight/ {print $2}' | tr -d ',' )
...
function cleanup {
  ...
  if $NETWORK_DISABLED; then
    ...
    ${GROESTLCOIN_CLI_CALL} setnetworkactive true
  fi
}
...
if [[ "$REPLY" =~ ^[Yy]*$ || -z "$REPLY" ]]; then
  ...
  ${GROESTLCOIN_CLI_CALL} setnetworkactive false
else
...
if [[ "${OUTPUT_PATH}" = "-" ]]; then
  ...
  ${GROESTLCOIN_CLI_CALL} gettxoutsetinfo | grep hash_serialized_3 | sed 's/^.*: "\(.\+\)\+",/\1/g'
else
